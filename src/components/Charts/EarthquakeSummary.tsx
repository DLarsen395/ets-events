...
import type { ApiSummaryStats } from '../../services/api';

interface EarthquakeSummaryProps {
  /** Summary stats from V2 API (optional) */
  apiStats?: ApiSummaryStats | null;
}

export function EarthquakeSummary({ apiStats }: EarthquakeSummaryProps) {
  ...
  // V2 API mode with stats
  if (isApiMode && apiStats) {
    return (
      <div style={containerStyle}>
        <h3 style={titleStyle}>Summary Statistics</h3>
        ...
        >
          <div style={statStyle}>
            <div style={valueStyle}>{apiStats.totalEvents.toLocaleString()}</div>
            <div style={labelStyle}>Total Events</div>
          </div>

          <div style={statStyle}>
            <div style={{ ...valueStyle, color: '#fbbf24' }}>
              M{apiStats.maxMagnitude?.toFixed(1) ?? '-'}
            </div>
          ...
          <div style={statStyle}>
            <div style={{ ...valueStyle, color: '#60a5fa' }}>
              M{apiStats.avgMagnitude?.toFixed(1) ?? '-'}
            </div>
          ...
          <div style={statStyle}>
            <div style={valueStyle}>
              {apiStats.avgDepth?.toFixed(1) ?? '-'}km
            </div>
        ...
        >
          <div style={statStyle}>
            <div style={{ ...valueStyle, fontSize: '1.25rem', color: '#f87171' }}>
              {apiStats.significantEvents}
            </div>
            <div style={labelStyle}>M5+ Events</div>
          </div>

          <div style={statStyle}>
            <div style={{ ...valueStyle, fontSize: '1.25rem', color: '#a78bfa' }}>
              {apiStats.majorEvents}
            </div>
            <div style={labelStyle}>M6+ Events</div>
          </div>

          <div style={statStyle}>
            <div style={{ ...valueStyle, fontSize: '1.25rem' }}>
              {apiStats.eventsPerDay}
            </div>
            <div style={labelStyle}>Per Day</div>
          </div>
        </div>

        {/* Largest Event */}
        {apiStats.largestEvent && (
          <div
            style={{
              marginTop: '0.75rem',
              padding: '0.5rem',
          ...
          >
            ...
            <div style={{ fontSize: '0.875rem', color: 'white' }}>
              M{apiStats.largestEvent.magnitude.toFixed(1)} -{' '}
              {apiStats.largestEvent.place || 'Unknown location'}
            </div>
            <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginTop: '0.25rem' }}>
              {format(new Date(apiStats.largestEvent.time), 'MMM d, yyyy HH:mm')} UTC
            </div>
          </div>
        )}
      </div>
  ...
  // V2 API mode without stats (loading or error)
  if (isApiMode && !apiStats) {
    return (
      <div style={containerStyle}>
        <h3 style={titleStyle}>Summary Statistics</h3>
